...
<div class="card shadow-sm border-0">
    <div class="card-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 py-3 px-4 bg-white border-bottom">
        <div>
            <h3 class="card-title fw-bold text-dark mb-1">Tipe & Periode Investasi</h3>
            <div class="text-muted small">Kelola skema pembagian hasil dan model periode investasi</div>
        </div>
        @can('buat type investasi')
            ...
            <div class="col-md-4 col-12">
                <form method="GET">
                    <div class="input-group input-group-sm">
                        <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Cari nama tipe investasi...">
                ...
                @forelse ($type as $item)
                    <tr>
                        <td class="text-center text-muted fw-semibold">
                            {{ $loop->iteration + ($type->currentPage() - 1) * $type->perPage() }}
                        </td>
                        <td>
                        ...
                        <td class="text-center">
                            <div class="action-btn-group">
                                @can('ubah type investasi')
                                    <a href="javascript:void(0)" onclick="return editModal('{{ $item->id }}')" class="btn-action btn-action-warning" title="Edit Tipe">
                                ...
                                @endcan
                                @can('hapus type investasi')
                                    <button type="button" onclick="return deleteType('{{ $item->id }}')" class="btn-action btn-action-danger" title="Hapus Tipe">
    ...
    <div class="table-footer d-flex flex-column flex-md-row align-items-center justify-content-between gap-3">
        <p class="m-0 text-muted small">
            Menampilkan <span class="fw-semibold text-dark">{{ $type->firstItem() ?? 0 }}</span> - <span class="fw-semibold text-dark">{{ $type->lastItem() ?? 0 }}</span> dari <span class="fw-semibold text-dark">{{ $type->total() }}</span> entri
        </p>
        <div class="m-0">
            {{ $type->links() }}
        </div>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content shadow-lg border-0">
            <div class="modal-header py-3 px-4 bg-white border-bottom">
                <h5 class="modal-title fw-bold text-dark">Tambah Type Investasi</h5>
            ...
            <div class="modal-body p-4">
                ...
                <div class="mb-3">
                    <label for="name" class="form-label">Nama Type Investasi <span class="text-danger">*</span></label>
                    <input type="text" name="name" id="name" class="form-control" placeholder="Contoh: Dari Dana Usaha">
...
<script>
    ...
    $("#addBtn").click(function() {
        $(".modal-title").html("Tambah Type Investasi");
        $("#name").val("");
        $("#type").val("create");
        $("#id").val("");
    });
    ...
    function editModal(id) {
        ...
        $.ajax({
            ...
            $(".modal-title").html("Edit Type Investasi");
    ...
    function deleteType(id) {
        Swal.fire({
            ...
            text: "Apakah Anda yakin ingin menghapus tipe investasi ini?",
            icon: "warning",
